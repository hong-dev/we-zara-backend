...
tag_id_list  = []
product_info = []
li_tag_list  = soup.select("#products > div._groups-wrap > ul > li")

for element in li_tag_list:
    tag_id_list.append("".join(element['id']))

product_pre_name  = ""
product_pre_color = ""

for element in tag_id_list:
    if tag_id_list.index(element) > 0:
        try:
            if 'product' in element:
                ...
                for element in img:
                    img_url.append("".join(element['src']))

                product_detail.append(img_url[0])
                if not(product_pre_color == product_detail[5] and product_pre_name  == product_detail[1]):
                    product_info.append(product_detail)
                    ...
                    data.to_csv('zara_sweater.csv', encoding='utf-8')
